...
export default async function (fastify: FastifyInstance) {
    ...
    }, async (request: FastifyRequest<{ Querystring: {total : true} }>, reply: FastifyReply) => {
        try {
            await AppDataSource.getRepository(Config).update({id: 1},{ abilitaTotale : request.query.total})
